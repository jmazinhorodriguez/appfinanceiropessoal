...
export function Header() {
  ...
  const getPageInfo = () => {
    ...
    if (pathname.startsWith('/investimentos')) return { title: 'Carteira de Investimentos', subtitle: 'Ativos ativos e histórico' };
    if (pathname.startsWith('/saude-aplicacoes')) return { title: 'Saúde das Aplicações', subtitle: 'Riscos e diversificação' };
    if (pathname.startsWith('/imposto-renda')) return { title: 'Imposto de Renda', subtitle: 'Apuração e declaração' };
    if (pathname.startsWith('/settings')) return { title: 'Configurações', subtitle: 'Ajustes da conta' };
  ...
  return (
    <header className="lg-header" style={{ 
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <button className="btn-glass" style={{ width: 44, padding: 0 }}>
          <Search size={18} />
        </button>
        <button className="btn-glass" style={{ width: 44, padding: 0, position: 'relative' }}>
          <Bell size={18} />
          <div style={{ 
